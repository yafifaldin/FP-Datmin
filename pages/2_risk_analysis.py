...
<p class="page-subtitle">
    Composite threat scoring across size, velocity, and proximity —
...
# ── Scatter ────────────────────────────────────────────────────────────────────
st.markdown('<p class="section-label">Velocity vs. miss distance</p>', unsafe_allow_html=True)
st.markdown("""
...
fig_scatter.add_trace(go.Scatter(
    ...
    hovertemplate="<b>%{text}</b><br>%{x:.4f} AU · %{y:.1f} km/s<extra></extra>",
))
fig_scatter.add_trace(go.Scatter(
    x=df.loc[haz_mask, "miss_distance_au"],
    y=df.loc[haz_mask, "velocity_kms"],
...
fig_scatter.update_layout(
    ...
    yaxis=dict(title="Velocity (km/s)", gridcolor="#161630",
               tickfont=dict(color="#ffffff"), zeroline=False),
    legend=dict(bgcolor="rgba(0,0,0,0)", font=dict(color="#ffffff", size=11), borderwidth=0),
...
# ── Top 15 ────────────────────────────────────────────────────────────────────
st.markdown('<p class="section-label">Top 15 by risk score</p>', unsafe_allow_html=True)
...
fig_top.update_layout(
    ...
    xaxis=dict(title="Risk Score", gridcolor="#161630",
               tickfont=dict(color="#ffffff"), zeroline=False),
    yaxis=dict(tickfont=dict(color="#ffffff", size=10), automargin=True),
...
<div class="callout">
    ...
    These objects carry high kinetic threat potential — large size, fast velocity,
    or close proximity — despite not meeting NASA's official hazard criteria.
</div>
""", unsafe_allow_html=True)
